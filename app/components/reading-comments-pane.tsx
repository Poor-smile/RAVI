...
export function ReadingCommentsPane(props: {
  ...
  return (
    <section ref={panelRef} id={panelId} className="sidebar-annotations-view reading-comments-pane" aria-labelledby="sidebar-pane-title" tabIndex={-1} onKeyDownCapture={(event) => { if (event.key !== "Escape") return; event.preventDefault(); event.stopPropagation(); onDismiss(); }}>
      ...
      {session.phase === "confirm" && <div className="smart-annotation-confirm" role="group" aria-label="تأیید نشانه‌گذاری هوشمند"><strong>کل سند بررسی می‌شود</strong><p>راوی متن همین سند را برای پیدا‌کردن ابهام، تناقض، تکرار و ضعف ساختار به Codex CLI می‌فرستد. هیچ تغییری بدون تأیید شما اعمال نمی‌شود.</p><div className="smart-annotation-privacy"><strong>محدوده: کل سند</strong><span>کامنت‌ها و نتیجه در فایل Markdown محلی می‌مانند.</span></div><div className="smart-annotation-actions"><button type="button" onClick={() => setPhase("idle")}>انصراف</button><button className="is-primary" type="button" onClick={() => void runReview()}>شروع بررسی</button></div><small>پیش از اعمال، نسخهٔ فعلی سند برای بازگشت احتمالی ذخیره می‌شود.</small></div>}
      ...
      {session.phase === "disconnected" && <div className="smart-annotation-state is-warning" role="status"><AlertTriangle size={21} aria-hidden="true" /><div><strong>Codex CLI آماده نیست</strong><small>نظرات عادی و نشانه‌های محلی همچنان در دسترس‌اند.</small></div><div className="smart-annotation-actions">{connectionState === "cli_missing" ? <button type="button" onClick={onOpenInstallGuide}>راهنمای نصب</button> : connectionState === "auth_required" ? <button type="button" onClick={() => void onStartLogin()}>ورود با ChatGPT</button> : <button type="button" onClick={() => void onCheckConnection()}>بررسی اتصال</button>}</div></div>}
