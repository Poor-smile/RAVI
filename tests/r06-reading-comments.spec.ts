...
test("R06 uses the modal Drawer and 44px supporting targets on mobile", async ({
  page,
}) => {
  await openReadingFixture(
    page,
    READING_COMMENTS_FIXTURE,
    "r06-reading-comments-mobile",
    THREE_COMMENTS,
    { width: 375, height: 812 },
  );

  const drawer = page.locator("#library-panel");
  const commentsTrigger = drawer.getByRole("button", {
    name: "نظرات",
    exact: true,
  });
  ...
  await drawer.getByRole("button", { name: "جست‌وجو در نظرات" }).click();

  const firstRow = drawer.locator(".reading-comment-row-jump").first();
  await firstRow.focus();
  const targets = await page.evaluate(() => {
    const dismiss = document.querySelector<HTMLElement>(
      ".reading-comments-search-control button",
    )!;
    const remove = document.querySelector<HTMLElement>(
      ".reading-comment-row-delete",
    )!;
    const row = document.querySelector<HTMLElement>(
      ".reading-comment-row-main",
    )!;
    const jump = document.querySelector<HTMLElement>(
      ".reading-comment-row-jump",
    )!;
    const body = document.querySelector<HTMLElement>(
      ".reading-comment-row-body",
    )!;
    ...
    return {
      ...
      jumpHeight: Math.round(jumpRect.height),
