(() => {
  ...
  const fallbackMarkdown = `# Markdown فارسی را ساده شروع کنید

اگر فایل معرفی از هاست در دسترس نباشد، این نمونهٔ داخلی همچنان امکان ویرایش و پیش‌نمایش را فعال نگه می‌دارد.

> متن را تغییر دهید؛ نتیجه همان لحظه در برگهٔ پیش‌نمایش دیده می‌شود.

## همین‌جا امتحان کنید

- ویرایش و پیش‌نمایش زنده
- نمایش درست فارسی و انگلیسی
- بدون ارسال محتوای نوشته

| قابلیت | نتیجه |
| --- | --- |
| جهت متن | فارسی راست‌به‌چپ و کد چپ‌به‌راست |
| حریم خصوصی | ویرایش فقط در همین مرورگر |
...
[باز کردن نسخهٔ وب راوی](https://raviweb.poorsmile.ir/)`;
  ...
  initialMarkdownRequest
    ...
    .catch(() => {
      initialMarkdown = fallbackMarkdown.trim();
      source.value = initialMarkdown;
